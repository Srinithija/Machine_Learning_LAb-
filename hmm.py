...
st.write("Predict hidden states using Viterbi Algorithm")

# Load dataset
@st.cache_data
def load_data():
    return pd.read_csv("hmm.csv")
...
# -----------------------------
# Compute Initial Probability π
# -----------------------------
pi = np.zeros(len(states))
first_state = data['HiddenState'][0]
pi[state_map[first_state]] = 1.0
